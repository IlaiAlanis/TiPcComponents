// assets/js/pages/admin_discounts.js
...
async function loadAdminProfile() {
    try {
        ...
        if (user) {
            const fullName = [user.nombreUsuario, user.apellidoPaterno, user.apellidoMaterno].filter(Boolean).join(' ');
...
async function loadDiscounts() {
    ...
    try {
        ...
        if (response.success && response.data) {
            let allDiscounts = response.data.items || response.data;
