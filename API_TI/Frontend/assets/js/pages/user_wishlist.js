...
async function loadWishlist() {
    ...
    if (response.success && response.data) {
        ...
        if (wishlistItems.length === 0) {
            container.innerHTML = `
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-500 dark:text-gray-400">No tienes productos en tu lista de deseos</p>
                    <a href="/pages/catalog.html" class="inline-block mt-4 px-6 py-2 bg-primary text-white rounded-lg hover:bg-primary-hover">
                        Ver Productos
                    </a>
                </div>
            `;
        ...
        container.innerHTML = '<div class="text-center py-12 text-red-500">Error al cargar la lista de deseos</div>';
    }
}

function createWishlistItem(item) {
    const product = item.producto;
    const imageUrl = product.imagenUrl || 'https://via.placeholder.com/80x80?text=Producto';
    ...
    const stockStatus = product.cantidadStock < 5 ? 'Stock bajo' : 'En stock';
    const stockColor = product.cantidadStock < 5 ? 'bg-orange-500/20 text-orange-400' : 'bg-green-500/20 text-green-400';

    return `
        <div class="py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div class="flex items-center gap-4 flex-1">
                <img src="${imageUrl}" alt="${product.nombreProducto}" class="w-20 h-20 object-cover rounded-lg flex-shrink-0"/>
                <div class="flex-1">
                    <h3 class="font-medium leading-tight mb-1">${product.nombreProducto}</h3>
                    <div class="flex items-center gap-2">
                        <p class="text-primary text-lg font-bold">$${price.toFixed(2)}</p>
                        ${hasDiscount ? `<p class="text-muted-light dark:text-muted-dark line-through text-sm">$${product.precioBase.toFixed(2)}</p>` : ''}
                    </div>
                    ${inStock ? `
                        <span class="inline-block mt-2 ${stockColor} text-xs font-medium px-2 py-1 rounded-full">
                            ${stockStatus}
                        </span>
                    ` : '<span class="inline-block mt-2 bg-red-500/20 text-red-400 text-xs font-medium px-2 py-1 rounded-full">Sin stock</span>'}
                </div>
            </div>
            <div class="flex items-center gap-4 w-full sm:w-auto">
                ...
                <button onclick="removeFromWishlist(${product.idProducto})" class="text-muted-light dark:text-muted-dark hover:text-red-500 transition-colors">
...
async function addAllToCart() {
    ...
    for (const item of wishlistItems) {
        if (item.producto.cantidadStock > 0) {
            const response = await API.cart.addItem({ productoId: item.producto.idProducto, cantidad: 1 });
